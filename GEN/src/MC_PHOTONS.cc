...
namespace Rivet {
  ...
  public:
    ...
    void init() {
      ...
      if ( getOption("MODE") == "PROMPT" ) {
        PromptFinalState prompt_leptons(leptons);
        ...
        declare(prompt_leptons, "lFS");
        declare(prompt_photons, "gammaFS");
      }
      else {
        declare(leptons, "lFS");
    ...
    void finalize() {
      scale(_h_Ptgamma, crossSection() / sumOfWeights());
      scale(_h_Egamma, crossSection() / sumOfWeights());
      scale(_h_sumPtgamma, crossSection() / sumOfWeights());
      scale(_h_sumEgamma, crossSection() / sumOfWeights());
      scale(_h_DelR, crossSection() / sumOfWeights());
