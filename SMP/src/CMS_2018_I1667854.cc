...
namespace Rivet {
  ...
  public:
    ...
    void init() {
      ...
      _h_zpt1 = bookHisto1D(3, 1, 1);
      _h_leading_jet_pt_tot = bookHisto1D(4, 1, 1);
      _h_second_jet_pt_tot = bookHisto1D(5, 1, 1);
      _h_third_jet_pt_tot = bookHisto1D(6, 1, 1);
      _h_leading_jet_y_tot = bookHisto1D(7, 1, 1);
      _h_second_jet_y_tot = bookHisto1D(8, 1, 1);
      _h_third_jet_y_tot = bookHisto1D(9, 1, 1);
      _h_ht1_tot = bookHisto1D(10, 1, 1);
      _h_ht2_tot = bookHisto1D(11, 1, 1);
      _h_ht3_tot = bookHisto1D(12, 1, 1);
      _h_ptbal1 = bookHisto1D(13, 1, 1);
      _h_ptbal2 = bookHisto1D(14, 1, 1);
      _h_ptbal3 = bookHisto1D(15, 1, 1);
      _h_jzb = bookHisto1D(16, 1, 1);
      _h_jzb_ptHigh = bookHisto1D(17, 1, 1);
      _h_jzb_ptLow = bookHisto1D(18, 1, 1);
    }
    ...
    void analyze(const Event& event) {;
      ...
      _h_zpt1->fill(z->pT(), weight);
      const Jet& j1 = goodjets[0];
      _h_leading_jet_pt_tot->fill(j1.pT()/GeV, weight);
      _h_leading_jet_y_tot->fill(j1.absrapidity(), weight);
      _h_ht1_tot->fill(ht/GeV, weight);
      _h_jzb->fill(jzb/GeV, weight);
      if(z->pT() > 50*GeV){
        _h_jzb_ptHigh->fill(jzb/GeV, weight);
      } else{
        _h_jzb_ptLow->fill(jzb/GeV, weight);
      }
      _h_ptbal1->fill(ptbal/GeV, weight);

      // Fill 2nd jet histograms
      if (goodjets.size() < 2) return;
      const Jet& j2 = goodjets[1];
      _h_second_jet_pt_tot->fill(j2.pT()/GeV, weight);
      _h_second_jet_y_tot->fill(j2.absrapidity(), weight);
      _h_ht2_tot->fill(ht/GeV, weight);
      _h_ptbal2->fill(ptbal/GeV, weight);

      // Fill 3rd jet histograms
      if (goodjets.size() < 3) return;
      const Jet& j3 = goodjets[2];
      _h_third_jet_pt_tot->fill(j3.pT()/GeV, weight);
      _h_third_jet_y_tot->fill(j3.absrapidity(), weight);
      _h_ht3_tot->fill(ht/GeV, weight);
      _h_ptbal3->fill(ptbal/GeV, weight);
    }
