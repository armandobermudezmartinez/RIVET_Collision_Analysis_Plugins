...
namespace Rivet {
  ...
  public:
	...
	//if(fabs(j.momentum().eta()) < 2.4 && j.momentum().pT()>20) {
	if(j.momentum().pT()>20) {
	...
	_h_LeadingBJetpt->fill(bjets[0].momentum().pT(),weight);
	_h_SubleadingBJetpt->fill(bjets[1].momentum().pT(),weight);
	_h_LeadingLightJetpt->fill(ljets[0].momentum().pT(),weight);
	_h_SubleadingLightJetpt->fill(ljets[1].momentum().pT(),weight);

	_h_LeadingBJeteta->fill(bjets[0].momentum().eta(),weight);
	_h_SubleadingBJeteta->fill(bjets[1].momentum().eta(),weight);
	_h_LeadingLightJeteta->fill(ljets[0].momentum().eta(),weight);
	_h_SubleadingLightJeteta->fill(ljets[1].momentum().eta(),weight);

	double lightdphi=deltaPhi(ljets[0].momentum().phi(),ljets[1].momentum().phi());
	_h_deltaphiafterlight->fill(lightdphi,weight);

	const double vecsumlightjets=sqrt(pow(ljets[0].momentum().px()+ljets[1].momentum().px(),2)+pow(ljets[0].momentum().py()+ljets[1].momentum().py(),2));
	
	const double term2=vecsumlightjets/(sqrt(ljets[0].momentum().px()*ljets[0].momentum().px()+ljets[0].momentum().py()*ljets[0].momentum().py())+sqrt(ljets[1].momentum().px()*ljets[1].momentum().px()+ljets[1].momentum().py()*ljets[1].momentum().py()));

	_h_SumPLight->fill(term2,weight);

	const double pxBsyst2=bjets[0].momentum().px()+bjets[1].momentum().px();
	const double pyBsyst2=bjets[0].momentum().py()+bjets[1].momentum().py();
	const double pxJetssyst2=ljets[0].momentum().px()+ljets[1].momentum().px();
	const double pyJetssyst2=ljets[0].momentum().py()+ljets[1].momentum().py();
