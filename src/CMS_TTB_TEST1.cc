...
class CMS_TTB_TEST1 : public Analysis
{
	private:
		Particles _muelphs;
		...
		//Histo1DPtr _hist_lpt;
		//Histo1DPtr _hist_nupt;
		//Histo1DPtr _hist_bpt;
		//Histo1DPtr _hist_jpt;
	...
	public:
		...
		/// Constructor
		CMS_TTB_TEST1()
			: Analysis("CMS_TTB_TEST1"), _jetetamax(2.5), _jetptmin(25.), _lepetamax(2.5), _lepptmin(30.)
		{    }
		...
		void init()
		{
			...
			//book hists
			_hist_thadpt = bookHisto1D("thadpt");
			_hist_thady = bookHisto1D("thady");
			_hist_tleppt = bookHisto1D("tleppt");
			_hist_tlepy = bookHisto1D("tlepy");
			_hist_ttpt = bookHisto1D("ttpt");
			_hist_tty = bookHisto1D("tty");
			_hist_ttm = bookHisto1D("ttm");
			_hist_njet = bookHisto1D("njet");
			//_hist_lpt = bookHisto1D("lpt");
			//_hist_nupt = bookHisto1D("nupt");
			//_hist_bpt = bookHisto1D("bpt");
			//_hist_jpt = bookHisto1D("jpt");
		...
		void analyze(const Event& event)
		{
			...
			for(vector<const HepMC::GenParticle*>::const_iterator itgenpar = allgenpars.begin() ; itgenpar != allgenpars.end() ; ++itgenpar)
			{
				...
				//if(PID::isHadron(pdgid) && PID::hasBottom(pdgid))
			...
			//_hist_lpt->fill(_dressLeps[0].pt(), weight);
			//_hist_nupt->fill(_nusum.pt(), weight);
			//_hist_bpt->fill(_bl->pt(), weight);
			//_hist_bpt->fill(_bh->pt(), weight);
			//_hist_jpt->fill(_wja->pt(), weight);
			//_hist_jpt->fill(_wjb->pt(), weight);
		...
		void finalize()
		{
			...
			//scale(_hist_lpt, crossSection()/sumOfWeights());
			//scale(_hist_nupt, crossSection()/sumOfWeights());
			//scale(_hist_bpt, crossSection()/sumOfWeights());
			//scale(_hist_jpt, crossSection()/sumOfWeights());
...
DECLARE_RIVET_PLUGIN(CMS_TTB_TEST1);
